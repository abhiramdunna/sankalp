    ...
    <>
      ...
      >
        ...
        <Modal
          ...
          onShow={() => {
            // Force keyboard open — autoFocus inside nested Modal is unreliable on Android
            setTimeout(() => qtyInputRef.current?.focus(), Platform.OS === 'android' ? 150 : 50);
          }}
        >
          <KeyboardAvoidingView
            style={{ flex: 1 }}
            behavior={Platform.OS === 'ios' ? 'padding' : 'height'}
          >
          <TouchableWithoutFeedback onPress={() => { Keyboard.dismiss(); setQtyModalVisible(false); }}>
            <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'flex-end' }}>
              <TouchableWithoutFeedback>
                <View style={{ backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingTop: 12, paddingBottom: 32 }}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: `${theme.colors.primary}0D`, borderRadius: 14, padding: 12, marginBottom: 20 }}>
                    <View style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: `${theme.colors.primary}20`, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
                      <Ionicons name="cube-outline" size={20} color={theme.colors.primary} />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 15, fontWeight: '800', color: '#111' }} numberOfLines={1}>
                        {qtyModalProduct?.name}
                      </Text>
                      <Text style={{ fontSize: 12, color: theme.colors.primary, fontWeight: '700', marginTop: 2 }}>
                        ₹{qtyModalProduct?.price.toFixed(2)} per unit
                      </Text>
                    </View>
                    {qtyModalVal.length > 0 && !isNaN(parseFloat(qtyModalVal)) && parseFloat(qtyModalVal) > 0 && (
                  ...
                  {/* Qty label + big input */}
                  <Text style={{ fontSize: 11, fontWeight: '700', color: '#9CA3AF', letterSpacing: 0.8, marginBottom: 8, textTransform: 'uppercase' }}>Quantity</Text>
                  <View style={{ borderWidth: 2.5, borderColor: theme.colors.primary, borderRadius: 16, paddingHorizontal: 16, paddingVertical: 10, backgroundColor: `${theme.colors.primary}06`, marginBottom: 16 }}>
                    <TextInput
                      ref={qtyInputRef}
                      style={{ fontSize: 32, fontWeight: '900', color: theme.colors.primary, textAlign: 'center', letterSpacing: 1 }}
                      value={qtyModalVal}
                  ...
                  <View style={{ flexDirection: 'row', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
                    {['0.25', '0.5', '1', '2', '5', '10', '25', '50'].map(preset => (
                      <TouchableOpacity
                        ...
                        style={{ paddingHorizontal: 14, paddingVertical: 8, backgroundColor: qtyModalVal === preset ? theme.colors.primary : '#F3F4F6', borderRadius: 20, borderWidth: 1.5, borderColor: qtyModalVal === preset ? theme.colors.primary : 'transparent' }}
                      >
                  ...
                  <View style={{ flexDirection: 'row', gap: 10 }}>
                    <TouchableOpacity
                      style={{ flex: 1, paddingVertical: 14, borderRadius: 14, borderWidth: 1.5, borderColor: '#FEE2E2', backgroundColor: '#FFF5F5', alignItems: 'center', justifyContent: 'center', gap: 4 }}
                      onPress={() => {
                    ...
                    <TouchableOpacity
                      style={{ flex: 3, paddingVertical: 14, borderRadius: 14, backgroundColor: theme.colors.primary, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 }}
                      onPress={() => {
                        if (qtyModalProduct) {
